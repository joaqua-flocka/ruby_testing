...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        output = verify_game.verify_input(min, max, valid_input)
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        min = verify_game.instance_variable_get(:@minimum)
        max = verify_game.instance_variable_get(:@maximum)
        output = verify_game.verify_input(min, max, invalid_input)
  ...
  describe '#display_turn_order' do
    ...
    before do
      ...
      allow(search_display_turn).to receive(:make_guess)
      allow(game_display_turn).to receive(:display_guess).with(search_display_turn)
      allow(search_display_turn).to receive(:update_range)
      allow(search_display_turn).to receive(:game_over?)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      initial_count = game_display_turn.instance_variable_get(:@guess_count)
      game_display_turn.display_turn_order(search_display_turn)
      new_count = game_display_turn.instance_variable_get(:@guess_count)
      expect(new_count).to eql(initial_count + 1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_display_turn).to receive(:make_guess)
      game_display_turn.display_turn_order(search_display_turn)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(search_display_turn).to receive(:update_range)
      game_display_turn.display_turn_order(search_display_turn)
    end

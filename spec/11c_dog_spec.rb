...
describe Dog do
  ...
  subject(:rosie) { described_class.new('Rosie', 'mastiff', 'black') }
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when using a shared method name' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(rosie).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(rosie).to be_sleeping
  end
end
